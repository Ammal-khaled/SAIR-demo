...
export default function Dashboard() {
  ...
  const hotspotSummary = Object.values(
    incidents.reduce((acc, report) => {
      ...
      current.count += 1;
      current.types[report.accidentType] =
        (current.types[report.accidentType] || 0) + 1;

      if (
        (priorityRank[report.priority] || 0) >
        (priorityRank[current.highestPriority] || 0)
      ) {
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener("sair-demo-reports-updated", fetchIncidents);
    };
  }, []);

  const handleUpdateStatus = async (id, newStatus) => {
    if (isDemoMode() || dataSource === "mock") {
      const updatedReports = updateDemoReport(
        id,
        { status: newStatus },
        `Dashboard changed status to ${newStatus.replace("_", " ")}.`,
      );

      setIncidents(updatedReports);
      setSelectedIncident(
        updatedReports.find((report) => report.id === id) || null,
      );
      return;
    }
  ...
  const filteredIncidents = incidents.filter((inc) => {
    ...
    const matchesSearch =
      ...
      String(inc.title ?? "")
        .toLowerCase()
        .includes(normalizedSearch) ||
      String(inc.locationName ?? "")
        .toLowerCase()
        .includes(normalizedSearch) ||
      String(inc.reporterName ?? "")
        .toLowerCase()
        .includes(normalizedSearch) ||
      String(inc.partyName ?? "")
        .toLowerCase()
        .includes(normalizedSearch) ||
      String(inc.driver ?? "")
        .toLowerCase()
        .includes(normalizedSearch) ||
      String(inc.accidentType ?? "")
        .toLowerCase()
        .includes(normalizedSearch) ||
      String(inc.type ?? "")
        .toLowerCase()
        .includes(normalizedSearch) ||
      String(inc.assignedUnit ?? "")
        .toLowerCase()
        .includes(normalizedSearch) ||
      String(inc.status ?? "")
        .toLowerCase()
  ...
  useEffect(() => {
    ...
    const firstMatch = filteredIncidents[0] || null;
    setSelectedIncident(firstMatch);
  }, [isSearchMode, normalizedSearch, filteredIncidents.length]);
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        ...
        <main className="flex-1 relative bg-[#F4F7FB] flex flex-col p-4 lg:p-6 gap-6 overflow-y-auto">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 shrink-0">
            <div>
              <h1 className="text-xl lg:text-2xl font-bold text-[#102033]">
                Command Dashboard
              </h1>
              <p className="text-xs text-[#64748B] font-medium">
                Portfolio demo using fictional accident reports.
              </p>
            </div>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {Object.entries(statusBreakdown).map(([status, count]) => (
                      <span
                        key={status}
                        className="text-xs font-bold text-gray-600 bg-gray-50 border border-gray-100 px-3 py-1.5 rounded-lg uppercase"
                      >
                        {status}: {count}
                      </span>
                    ))}
                  ...
                  <div className="space-y-3">
                    {hotspotSummary.map((item) => (
                      <div
                        ...
                        className="border border-slate-100 rounded-xl p-3 bg-[#F4F7FB]"
                      >
          ...
          {isSearchMode && (
            <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-4 lg:p-5 shrink-0">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  <h2 className="text-base lg:text-lg font-bold text-[#102033]">
                    Search Results
                  </h2>
                  <p className="text-xs text-[#64748B] font-medium mt-1">
                    Showing {filteredIncidents.length} matching report
                    {filteredIncidents.length === 1 ? "" : "s"} for “
                    {searchQuery}”.
                  </p>
                </div>
                ...
                <button
                  ...
                  className="inline-flex items-center justify-center px-4 py-2 bg-blue-50 border border-blue-100 text-[#2563EB] rounded-xl text-xs font-bold uppercase hover:bg-blue-100 transition-colors"
                >
                  Clear search
                </button>
              </div>
          ...
          <div
            ref={containerRef}
            className="flex flex-col lg:flex-row relative min-h-0"
          >
            <div
              style={{
                width: window.innerWidth < 1024 ? "100%" : `${leftWidth}%`,
              }}
            ...
            >
              ...
              <div className="space-y-3 pb-4 custom-scrollbar pr-1">
                {filteredIncidents.length > 0 ? (
                  ...
                  <div className="text-center text-gray-400 text-sm py-8 bg-white border border-slate-100 rounded-2xl">
            ...
            <div
              ref={detailsRef}
              className="flex-1 bg-white rounded-2xl border border-gray-200 flex flex-col overflow-hidden shadow-sm lg:ml-2 min-h-[500px] mb-8 lg:mb-0"
            >
              {selectedIncident?.id ? (
                          ...
                          <>
                            ...
                            {(selectedIncident.status === "submitted" ||
                              ...
                              <button
                                ...
                                className="flex-1 sm:flex-none px-4 py-2 bg-[#F2A93B] hover:bg-[#e29b2f] transition-colors text-[#102033] font-bold text-sm rounded-xl shadow-md"
                              >
                            ...
                            {selectedIncident.status === "under_review" && (
                              <button
                                ...
                                className="flex-1 sm:flex-none px-4 py-2 bg-indigo-600 hover:bg-indigo-700 transition-colors text-white font-bold text-sm rounded-xl shadow-md"
                              >
                            ...
                            {selectedIncident.status === "approved" && (
                              <button
                                ...
                                className="flex-1 sm:flex-none px-4 py-2 bg-emerald-600 hover:bg-emerald-700 transition-colors text-white font-bold text-sm rounded-xl shadow-md"
                              >
                      ...
                      <div>
                        ...
                        <div className="bg-gray-50 rounded-xl p-4 lg:p-5 border border-gray-100 space-y-4">
                          ...
                          <div className="flex justify-between border-b border-gray-200 pb-3">
                            ...
                            <span className="font-bold text-[#1a4b7c] bg-blue-50 px-2 py-0.5 rounded text-sm uppercase">
                          ...
                          <div className="flex justify-between border-b border-gray-200 pb-3">
                            ...
                            <span className="font-bold text-gray-900 uppercase text-sm">
                              {selectedIncident.assignedUnit || "N/A"}
                            </span>
                          </div>
                      ...
                      selectedIncident.mediaUrls.length > 0 ? (
                        <div className="space-y-4">
                          {selectedIncident.mediaUrls.map((url, idx) => (
                            <div
                              key={idx}
                              className="rounded-xl overflow-hidden border border-gray-200 bg-gray-50 shadow-sm relative group cursor-pointer"
                            >
                              <img
                                ...
                                onClick={() =>
                                  window.open(
                                    `http://sair-cpa-api.duckdns.org${url}`,
                                    "_blank",
                                  )
                                }
                              />
